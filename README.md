...
After generating the matmolQUERY table, the project uses sentiment analysis to gauge the "mood" of each response. This sentiment score is then used to influence the weight of the neighboring cells in the table, creating a more nuanced final output that takes into account not only the content of the responses but also their sentiment.

## Summary Generation

Finally, the project summarizes the weighted matmolQUERY table into a single output. This summary takes into account both the content and the sentiment of the various responses, providing a balanced and nuanced answer.

## Explanation

In the context of this project, the term "additive" refers to the method used to aggregate or combine the sentiment scores of different responses generated by the AI model. This process is akin to creating a "multiplication table" of weights for each response, hence the term "matmolQUERY" (multiplication-multiplexed) table.
...
The AI model generates a response for each cell in the matmolQUERY table. The sentiment of each response is then evaluated using a sentiment analysis tool. The resulting sentiment score is a number between -1 and 1, representing the range from negative to positive sentiment.

Now comes the additive part. The sentiment score of each response is used to adjust the weights of its neighboring cells in the matmolQUERY table. Specifically, the sentiment score is added to the weight of each neighboring cell. This has the effect of "spreading" the sentiment of each response to its neighbors, hence creating a kind of "multiplication" effect.

The final summary response is generated by combining all the responses in the matmolQUERY table, taking into account their adjusted weights. In this way, the summary response is a weighted combination of all the original responses, where the weights are influenced by the sentiment scores of each response and its neighbors.
...
This is a summary of how this code works:

- It imports some libraries and modules, such as openai, numpy, json, nltk, dotenv, and os. These are used for various purposes, such as generating chatbot responses, performing sentiment analysis, saving data to json files, and accessing environment variables.
- It defines some functions, such as get_sentiment, generate_chat_model, matmol_responses, get_final_summary, save_to_json, and main. These are used for different steps of the method, such as getting the sentiment score of a text, generating a chatbot response to a prompt, creating a grid matrix of responses, summarizing the responses based on their sentiment and weights, saving the data to a json file, and running the main program.
- It asks the user to input a prompt and a size for the matmolQUERY table. The prompt is the question or topic that the chatbot will respond to. The size is the number of rows and columns in the grid matrix of responses.
- It calls the matmol_responses function to generate a list of responses for each position in the grid matrix. The number of responses depends on whether the position is at the edge or not. The responses are generated using the generate_chat_model function, which uses the openai.Completion endpoint and the text-davinci-002 engine to create chatbot responses with a given temperature and max_tokens.
- It creates an empty list of summaries with the same size as the grid matrix. Then it fills in the first response at each position as the summary for that position.
- It calls the get_final_summary function to create a final summary text based on the summaries and responses. It iterates over each summary and response, and adjusts the weights of the neighboring positions based on their sentiment scores. Then it concatenates the first response at each position that has a positive weight and appends it to the final summary. It also adds another response generated from the final summary itself using the generate_chat_model function.
- It prints out the final summary and saves the data to a json file using the save_to_json function. The data includes the prompt, responses, summaries, and final summary.
- It runs the main function when the script is executed.
...
# Multiplication in LLM with matmolQUERY addition tables
